...
insert into tb_track_statistics_hour(id,data_type,first_time,first_device_id,first_device_type,first_info_id,last_time,last_device_id,last_device_type,last_info_id,cnt,pt)
select id,data_type,first_time,first_device_id,first_device_type,first_info_id,last_time,last_device_id,last_device_type, last_info_id,sum(cnt) cnt,@BATCH_TIME@ as pt  from(
select id,data_type,
first_value(first_time)over(partition by id,data_type order by pt  asc RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING ) first_time,
...
cnt
from tb_track_statistics_min where pt >= @START_TIME_HOUR@ and pt < @BATCH_TIME@
