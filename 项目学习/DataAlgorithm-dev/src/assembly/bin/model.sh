...
JARS="$JAR_PATH/fastjson-1.2.49.jar,$JAR_PATH/slf4j-api-1.7.25.jar,$JAR_PATH/jest-common-5.3.3.jar,$JAR_PATH/jest-5.3.3.jar,$JAR_PATH/httpcore-nio-4.4.6.jar,$JAR_PATH/httpasyncclient-4.1.3.jar,$JAR_PATH/mysql-connector-java-5.1.34.jar,$JAR_PATH/postgresql-9.4.1212.jar,$JAR_PATH/druid-1.0.15.jar,$JAR_PATH/snowball-jdbc-2.4.0.jar"
#执行模式：默认是yarn-client
execMode=yarn
...
spark-submit --class com.suntek.algorithm.process.main \
   ...
   --driver-cores 1 \
   --num-executors 3 \
   --conf "spark.driver.extraJavaOptions=-XX:PermSize=2g -XX:MaxPermSize=2g -Dlog4j.configuration=file://${driverLog4jPath} -DmainClass=${mainClass} -DrelType=${rel_type}" \
   ...
   --conf "spark.sql.sources.parallelPartitionDiscovery.threshold=2000" \
